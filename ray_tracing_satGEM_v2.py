...
def plane_wave(k, l, m, omega, f, t, p0, N2):
    ...
    b0 = no.absolute(p0*(m*N2)/(N2 - omega**2))
    # p0 = 
    ...
    return u0, v0, w0, uprime, vprime, wprime, bprime
...
#            t_idx, clon_idx, clat_idx = satGEM.locate(lon, lat, z, t)
        ...
        # k step
        k = k + del_wavenum(dudx, dvdx, k, l, m, np.sqrt(N2), dndx, Omega)*tstep

        # l step
        l = l + del_wavenum(dudy, dvdy, k, l, m, np.sqrt(N2), dndy, Omega)*tstep
    
        # m step
        m = m + del_wavenum(dudz, dvdz, k, l, m, np.sqrt(N2), dndz, Omega)*tstep
        ...
        Omega = Omega + dOmega(rx, ry, rz, k, l, dudt, dvdt)*tstep
...
#            lonlims, latlims, tlims = gem.interpfield_lnd(lon, lat, z, t)
    ...
    if bathy:
        ...
        if energy:
            ...
            results['m_flux'] = action * (np.diff(results['omega'], axis=0) / tstep)
    ...
    # calculate plane wave perturbations and wave action and momentum fluxes
    # plane_waves = plane_wave(k , l, m)
    # results['u0'] = u0
    # results['v0'] = v0
    # results['w0'] = w0
    # results['b0'] = b0
    # results['uprime'] = uprime
    # results['vprime'] = vprime
    # results['wprime'] = wprime
    # results['bprime'] = bprime
